...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<TraderInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/traders` : null),
    () => getTraders({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Traders',
      path: isTenantUser ? `/traders/view/${data?.[0]?.id}` : '/traders',
      entity: 'trader',
