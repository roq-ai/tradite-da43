...
import { getTraders, deleteTraderById } from 'apiSdk/traders';
import { TraderInterface } from 'interfaces/trader';

type ColumnType = ColumnDef<TraderInterface, unknown>;

interface TraderListPageProps {
...
export function TraderListPage(props: TraderListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTraders({
        relations: ['user', 'journal_entry.count', 'simulation.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TraderInterface>>(
    () => `/traders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TraderInterface) => {
    if (hasAccess('trader', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/traders/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('journal_entry', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'journal_entry',
          header: 'Journal Entry',
          accessorKey: 'journal_entry',
          cell: ({ row: { original: record } }: any) => record?._count?.journal_entry,
        }
      : null,
    hasAccess('simulation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.simulation,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/traders/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('trader', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/traders/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('trader', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="trader" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Traders
        </Text>
        {hasAccess('trader', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/traders/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
