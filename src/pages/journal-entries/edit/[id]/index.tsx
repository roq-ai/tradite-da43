...
import { getJournalEntryById, updateJournalEntryById } from 'apiSdk/journal-entries';
...
function JournalEntryEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<JournalEntryInterface>(
  ...
  const handleSubmit = async (values: JournalEntryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateJournalEntryById(id, values);
      mutate(updated);
      resetForm();
