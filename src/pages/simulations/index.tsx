...
type ColumnType = ColumnDef<SimulationInterface, unknown>;
...
export function SimulationListPage(props: SimulationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SimulationInterface>>(
    () => `/simulations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SimulationInterface) => {
    if (hasAccess('simulation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/simulations/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/simulations/view/${record.id}`} passHref legacyBehavior>
            <Button
